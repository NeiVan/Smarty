...
if($act=='profile'){
   ...
   if(!Common::is_mobile($data['phone'])){
      $res['msg'] = '手机号格式不正确';
   	  ...
   	  $res['msg'] = '请选择用户所属分组';
   }else if($Db->query("select pk_user_main from ".$Base->table('user')." where phone ='".$data['phone']."' and pk_user_main !=$id","One")){
      $res['msg'] = '手机号已被使用';
   }
   else{
      ...
      $uid = $Db->query("select pk_user_main from ".$Base->table('user')." where pk_user_main=$id","One");
	  //编辑
	  if($id){
	      $Db->update($Base->table('user'),$data,array('pk_user_main'=>$id));
	  ...
	  //添加
	     ...
	     if(empty($data['password'])){
			...
			$id = $Db->insert($Base->table('user'),$data);
			$res = array('status'=>1,'msg'=>'添加成功','href'=>'/'.ADMIN_PATH.'/?m=flynumber');
		 }
	  }
   }
